...
from salesforce_prototype_cdk.constructs.sp_repo \
    import SalesforcePrototypeEcrRepoProperties, SalesforcePrototypeEcrRepoConstruct
from salesforce_prototype_cdk.constructs.sp_storage \
    import SalesforcePrototypeStorageProperties, SalesforcePrototypeStorageConstruct
from salesforce_prototype_cdk.constructs.sp_snowflake_role \
    import SalesforcePrototypeSnowflakeRoleProperties, SalesforcePrototypeSnowflakeRoleConstruct
from salesforce_prototype_cdk.constructs.sp_parameters \
    import SalesforcePrototypeParametersProperties, SalesforcePrototypeParametersConstruct
from salesforce_prototype_cdk.constructs.sp_notifications \
    import SalesforcePrototypeNotificationProperties, SalesforcePrototypeNotificationConstruct
from salesforce_prototype_cdk.constructs.sp_task \
    import SalesforcePrototypeTaskProperties, SalesforcePrototypeTaskConstruct


class SalesforcePrototypeAppProperties(StackProps):
    """
    Configuration values for creating an instance of the SalesforcePrototypeAppStack class.
    """
    ...
    def __init__(self,
        ...
        """
        Create configuration values for creating an instance of the SalesforcePrototypeAppStack class.
...
class SalesforcePrototypeAppStack(Stack):
    """
    A stack that represents a deployment of the application into a single environment.
    """

    def __init__(self, scope: Construct, construct_id: str, properties: SalesforcePrototypeAppProperties,
                 **kwargs) -> None:
        """
        ...
        properties : SalesforcePrototypeAppProperties
        ...
        repo_properties = SalesforcePrototypeEcrRepoProperties(
        ...
        repo = SalesforcePrototypeEcrRepoConstruct(self, 'repo', properties=repo_properties)

        # S3 Bucket

        storage_properties = SalesforcePrototypeStorageProperties(base_name=properties.base_name,
                                                          target_account_id=properties.environment.account_id)
        bucket = SalesforcePrototypeStorageConstruct(self, 'storage', properties=storage_properties)

        # Snowflake Role

        snowflake_role_properties = SalesforcePrototypeSnowflakeRoleProperties(
        ...
        SalesforcePrototypeSnowflakeRoleConstruct(self, 'snowflake-role', properties=snowflake_role_properties)

        # Systems Manager Parameters

        parameters_properties = SalesforcePrototypeParametersProperties(base_name=properties.base_name,
                                                                parameter_definitions=properties.parameter_definitions,
                                                                parameter_values=properties.parameter_values)
        parameters = SalesforcePrototypeParametersConstruct(self, 'parameters', properties=parameters_properties)
        ...
        if notifications_email_to is not None and len(notifications_email_to) > 0 and \
            ...
            notification_properties = SalesforcePrototypeNotificationProperties(
                base_name=properties.base_name,
                subscription_email_address=properties.environment.settings.notifications_email_address)

            notification_topic = SalesforcePrototypeNotificationConstruct(self, "bi-notifications",
                                                                  properties=notification_properties)
            notification_topic = notification_topic.topic

        # ECS Task Definition

        task_properties = SalesforcePrototypeTaskProperties(base_name=properties.base_name,
        ...
        SalesforcePrototypeTaskConstruct(self, 'task', properties=task_properties)
